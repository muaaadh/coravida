#!/bin/bash
# ==========================================================================
# CORAVIDA — the film library.   bash tools/film.sh [name …]   |   --posters
...
CLIPS=(
  ...
  "vessel|$D/dji_fly_20260828_090835_0_1788104951519_video_cache.mp4|3|10|1080|30"
  "anchor|$D/dji_fly_20260828_085913_0_1788104951518_video_cache.mp4|0|6.5|1080|30"
  "boat-drift|$D/dji_fly_20260828_085652_0_1788104951514_video_cache.mp4|0|8.6|1080|30"
  "boat-blue|$D/dji_fly_20260828_085742_0_1788104951518_video_cache.mp4|0|8|1080|30"
...
if [ "$1" = "--posters" ]; then for c in "${CLIPS[@]}"; do IFS='|' read -r n _ _ _ m _ <<< "$c"; poster "$n" "$m"; echo "  poster-$n"; done; exit 0; fi
if [ $# -gt 0 ]; then for want in "$@"; do for c in "${CLIPS[@]}"; do [ "${c%%|*}" = "$want" ] && one "$c"; done; done
else for c in "${CLIPS[@]}"; do one "$c"; done; fi
