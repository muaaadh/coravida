...
window.DB = (function () {
  ...
  var records = {
    all: function () {
      ...
      function page() {
        return sb.from("records").select("id, kind, updated, deleted, data").order("updated", { ascending: true }).range(from, from + PAGE - 1).then(fail).then(function (r) {
    ...
    watch: function (onRow) {
      var ch = sb.channel("records-live").on("postgres_changes", { event: "*", schema: "public", table: "records" }, function (p) { if (p.new && p.new.id) onRow(p.new); }).subscribe();
